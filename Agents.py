...
class SARSA_LSFA():
    ...
    def plot(self,type,save_data):
        if type == 'train':
            ...
            fig.suptitle('SARSA LSFA {} Agent -  Last {:,} Avg : {:.2f}%'.format(self.strat_label,self.mean_window,win_avg[-1]),fontweight='bold',fontsize = 16)
    ...
    # while (self.check_stable == True or train_iters ==1):
        ...
        with open('Data/SARSA_LSFA/RunInfo/' + self.strat_label + self.label + '_Info.txt','a') as data: data.write('\n\nRun Time: {:.2f} seconds'.format(run_time))
...
#                 # SARSA tabular update
#                 TD_error = (reward + self.gamma * self.Q_val[state_prime[0],state_prime[1],state_prime[2],action_prime-1]
#                     -self.Q_val[state[0],state[1],state[2],action-1])

#                 self.Q_val[state[0],state[1],state[2],action-1] = self.Q_val[state[0],state[1],state[2],action-1] + self.alpha*TD_error
...
#         if run_time < 60: print('--- {:.2f} seconds ---'.format(run_time))
#         else: print('--- {:.2f} minutes ---'.format(run_time/60))
#         with open('Data/SARSA_LSFA/RunInfo/' + self.strat_label + self.label + '_Info.txt','a') as data: data.write('Run Time: {:.2f} seconds'.format(run_time))
